/*
 * minibash - Phase 4: If/Elif/Else + For Loops + Break/Continue
 *
 * Builds on Phase 3.  New in this phase:
 *   - run_body()           — execute all statements in a block, respecting break/continue
 *   - run_if_statement()   — if / elif / else / fi
 *   - run_for_statement()  — for VAR in list; do ... done
 *   - exec_exception_t     — EXEC_BREAK / EXEC_CONTINUE flags
 *   - "break" and "continue" builtins
 *   - ":" (colon/noop) builtin
 *   - test_command / negated_command support
 *
 * Passes all Phase 1+2+3 tests plus:
 *   090-if-statement-1              (if true; then echo true is true; fi)
 *   091-if-statement-2              (if/else)
 *   092-if-statement-3              (if/elif/else with [ -x ... ])
 *   095-if-statement-multiple-elif  (multiple elif chains)
 *   100-for-loop-1                  (for a in 1 2 3 4 "done")
 *   104-for-loop-break              (for with break via && short circuit)
 */
...
/* =========================================================================
 * NEW IN PHASE 4: Loop exception flag
 *
 * break and continue can't simply return — they need to propagate
 * up through run_body() until the enclosing loop catches them.
 *
 * When a builtin sets exec_exception, run_body() stops executing
 * statements.  The loop (run_while / run_for) checks for EXEC_BREAK
 * to exit and EXEC_CONTINUE to skip to the next iteration.
 * ========================================================================= */
...
/* =========================================================================
 * String expansion  (unchanged from Phase 3)
 * ========================================================================= */
...
/* =========================================================================
 * build_argv / free_argv  (unchanged)
 * ========================================================================= */
...
/* =========================================================================
 * apply_redirects  (unchanged from Phase 3)
 * ========================================================================= */
...
/* =========================================================================
 * Builtins (Phase 4: added break, continue, :)
 * ========================================================================= */
...
static bool try_builtin(char **argv)
{
    ...
    /* break: signal the enclosing loop to exit */
    if (strcmp(argv[0], "break") == 0) {
        exec_exception = EXEC_BREAK;
        last_exit_status = 0;
        return true;
    }
    /* continue: signal the enclosing loop to skip to next iteration */
    if (strcmp(argv[0], "continue") == 0) {
        exec_exception = EXEC_CONTINUE;
        last_exit_status = 0;
        return true;
    }
    /* : (colon) — the no-op command, always succeeds */
    if (strcmp(argv[0], ":") == 0) {
        last_exit_status = 0;
        return true;
    }
...
/* =========================================================================
 * run_simple_command  (unchanged from Phase 3)
 * ========================================================================= */
...
/* =========================================================================
 * run_pipeline  (unchanged from Phase 3)
 * ========================================================================= */
...
/* =========================================================================
 * run_redirected_statement  (unchanged from Phase 3)
 * ========================================================================= */
...
/* =========================================================================
 * NEW IN PHASE 4: run_body
 *
 * Execute all statements in a body node (do_group, compound_statement).
 * Stops early if exec_exception is set (break or continue was called).
 *
...
/* =========================================================================
 * NEW IN PHASE 4: run_if_statement
 *
 * Tree structure:
 *   if_statement
 *     condition: <list or command>
 *     body: <compound_statement>      ← the "then" block
 *     elif_clause*
 *     else_clause?
 *
 * Logic:
 *   1. Run the condition.
 *   2. If exit status == 0, run the body.
 *   3. Otherwise, try each elif_clause's condition.
 *   4. If no elif matched, run the else_clause body.
 * ========================================================================= */
...
/* =========================================================================
 * NEW IN PHASE 4: run_for_statement
 *
 * Tree structure:
 *   for_statement
 *     variable: word           ← loop variable name
 *     value*: word/string/...  ← items to iterate
 *     body: do_group
 *
 * Logic:
 *   1. Extract variable name.
 *   2. Collect all value children.
 *   3. For each value: expand it, store in shell_vars, run body.
...
run_for_statement(TSNode node)
{
    ...
    /* Collect values (everything that isn't the variable, body, or do_group) */
    uint32_t nc = ts_node_named_child_count(node);
...
/* =========================================================================
 * run_node — dispatcher (Phase 4: added if, for, do_group,
 *            compound_statement, negated_command, test_command)
 * ========================================================================= */
...
run_node(TSNode node, bool background)
{
    ...
    /* test_command: [ expr ] — tree-sitter wraps this; just run as command */
    if (strcmp(type, "test_command") == 0) {
        /* Build the full test command string and run via sh -c */
